...
# Example input:
#   00a369b4ec1e5e0ff34e6bd838e5f2d6_L_MLO_bbox-1_BI-RADS-3_Mass.nii.gz

filename=$1

# Extract patient ID (first field before '_')
patid=$(echo "${filename}" | cut -d'_' -f1)

# Extract view type (second and third fields joined by '_')
type=$(echo "${filename}" | cut -d'_' -f2-3)
...
# Path settings
fixed_image_base="/workspace/data/VinDr-Mammo/Mass/image"
moving_image_base="/workspace/data/VinDr-Mammo/Normal/image"
output_base="/workspace/data/VinDr-Mammo/Mass/registered_oppositeSide_image"
tmp_base="/workspace/data/VinDr-Mammo/tmp-mass"
error_log="/workspace/data/VinDr-Mammo/Error_Registration_Mass.txt"
